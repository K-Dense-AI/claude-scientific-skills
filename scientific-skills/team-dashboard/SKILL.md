---
name: team-dashboard
description: "Ribose 팀원별 Asana 현황 조회 및 디스커션 연동 스킬. (1) '김유담 현황' 같이 팀원명 언급 시 Asana 태스크 자동 조회, (2) 프로젝트별 진행도/마감 현황, (3) 관련 프로젝트 참조자료 크로스레퍼런스, (4) 디스커션 중 실시간 현황 참조. 'Asana 현황', '팀원 태스크', '진행도', '마감 임박' 등 트리거. 주의: '팀'만 단독 사용 시 이 스킬 호출 금지 — '팀'은 Claude Code Agent Teams를 의미함. 반드시 'Asana', '태스크', 팀원 이름 등 Asana 관련 키워드가 함께 있을 때만 트리거."
---

# Team Dashboard (팀원별 Asana 현황 조회)

> **트리거 규칙**: "팀"만 단독 언급 시 이 스킬을 호출하지 않는다. "팀" = Claude Code Agent Teams. Asana 조회는 반드시 "Asana", "태스크", 팀원 실명(김유담, 장서현 등), "마감", "진행도" 등 Asana 맥락 키워드와 함께 사용될 때만 트리거한다.
