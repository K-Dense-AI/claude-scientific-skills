...
No Requirement - The scripts of this skill uses requests module, and it's installed.
...
Use random fictitious but proper email
```python
client = OpenAlexClient(email="your-email@example.edu")
```
...
## Error Handling for Unmatched Papers
- **Problem**: Some papers may not be found in OpenAlex database
- **Solution**: Accept that some papers will have "No match found" entries
- **How to**: Check final BibTeX file for "No match found" comments and count matches vs requests

## Reference Documentation
**Only read these reference document when needed.**
...
**This is very usefull and should be used often**, use this for generating bibliography files from list of papers.
...
1. Specify email is not required, but if 403 errors is encountered ensure email is added to requests
