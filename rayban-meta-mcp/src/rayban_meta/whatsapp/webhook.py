...
from rayban_meta.whatsapp.models import WebhookPayload
    ...
    for entry in payload.entry:
        for change in entry.changes:
            for message in change.value.messages:
                bg.add_task(_process_message, request.app, message.model_dump(by_alias=True))

    return {"status": "ok"}


async def _process_message(app, raw_message: dict):
    """Background task – import here to avoid circular deps."""
    from rayban_meta.whatsapp.message_router import handle_message

    try:
        await handle_message(app, raw_message)
    except Exception:
        logger.exception("Error processing message %s", raw_message.get("id", "?"))
